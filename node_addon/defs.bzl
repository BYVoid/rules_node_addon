...
def _napi_version_local_defines(napi_version):
    if napi_version == None or napi_version == "":
        return []
    return ["NAPI_VERSION=%s" % napi_version]
...
def node_addon(
    ...
    Args:
      ...
      napi_version: Optional Node-API version to define for this addon target.
    ...
    cc_binary(
        ...
        linkshared = True,
        local_defines = local_defines + _napi_version_local_defines(napi_version),
        tags = tags,
        testonly = testonly,
        deps = deps + select({
            _WINDOWS: [
